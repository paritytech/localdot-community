...
Deployment is **manual / scripted** — this repo ships no CI. There are two paths, both publishing the static web bundle to the Bulletin Chain via `bulletin-deploy` and pointing a dotNS domain at the resulting CID.
...
MNEMONIC="..." bulletin-deploy --env paseo-next-v2 ./apps/web/dist <domain>.dot
```
...
| **Publish** | `bulletin-deploy --env paseo-next-v2` uploads `apps/web/dist` to the Bulletin Chain and points the dotNS domain at the new CID. `--env paseo-next-v2` selects the correct Paseo Next v2 chains |
...
npm install -g bulletin-deploy@latest
MNEMONIC="..." bulletin-deploy --env paseo-next-v2 './apps/web/dist' "localdot.dot"
```

> The single-command deploy (`pnpm run deploy`) has shipped and is the recommended path — it wraps the contract deploy, the build, and the `bulletin-deploy` publish above. Use the manual commands here only when you want to run an individual step on its own.
...
- [ ] `MNEMONIC` available to the deploy (passed in-memory to `bulletin-deploy`; never committed)
