...
2. **`polkadot-app-deploy`** (`npm install -g @parity/polkadot-app-deploy@latest`) — the tool the live deploy uses. The `dotns-sdk` CLI (https://github.com/paritytech/dotns-sdk) is an optional lower-level alternative.
...
> `polkadot-app-deploy --env paseo-next-v2` flag selects the correct Next chains for
...
> `polkadot-app-deploy --env paseo-next-v2` tool — not the `dotns-sdk` CLI directly.
